 ...
 * satisfy the requirements to be a straight or not, and if it beats the last hand on table.
 * 
...
public class Straight extends Hand {
	...
	public boolean isValid() {
		...
		for (int i = 0; i<this.size(); i++) {
			if (i>0) {
				if (this.getCard(i).compareTo(this.getCard(i-1)) == 1) {
					if (this.getCard(i).getRank() == 0) {
						if (this.getCard(i-1).getRank() != 12) {
							check = false;
						}
					}
					else {
	 ...
	 * 
	 * @return Straight or null		a string of the type of the given set of card(s) in hand (e.g. single, pair, triple, etc.)
	 */
	@Override
